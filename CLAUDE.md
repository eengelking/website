...
- **Branch cleanup is expected, not optional, once a PR merges.** After confirming the PR actually merged into `main` (don't assume from a chat message alone — verify with `gh pr view`), delete both copies of the branch: the local one (`git branch -d <branch>`) and the remote one (`git push origin --delete <branch>`), since GitHub doesn't auto-delete on merge in this repo. Leaving the remote branch around after a confirmed merge is the thing to avoid, not the default to fall back to. Deleting the remote branch is still a shared-state change, so confirm with Ed before doing it, same as any other push — but the expectation is that both copies get cleaned up as the normal close-out step for every merged PR, not treated as optional housekeeping.
